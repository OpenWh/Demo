...
function login(n) {
    ...
    t = t + "#" + n,
    location.href = "https://passport.cnblogs" + getHostPostfix() + "/login.aspx?ReturnUrl=" + encodeURIComponent(t),
    !1
}
function logout() {
    return confirm("确认退出吗？") && (location.href = "https://passport.cnblogs" + getHostPostfix() + "/logout.aspx?ReturnUrl=" + location.href),
    !1
}
function register() {
    return location.href = "https://passport.cnblogs" + getHostPostfix() + "/register.aspx?ReturnUrl=" + location.href,
    !1
}
...
function shBushPathPrepare() {
    for (var t = arguments,
    i = [], n = 0; n < t.length; n++) i.push(t[n].replace("@", location.protocol + "//common.cnblogs.com/script/sh/"));
    return i
}
...
function loadPageBeginHtml() {
    currentBlogApp && $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function (n) {
            n && (n.indexOf("<script") > -1 ? $.getScript(location.protocol + "//common.cnblogs.com/script/jquery.writeCapture-min.js",
            function () {
...
function loadBlogNews() {
    $.ajax({
        ...
        dataType: "text",
        success: function (n) {
            n && (n.indexOf("<script") < n.indexOf("<script>getFollowStatus") ? $.getScript(location.protocol + "//common.cnblogs.com/script/jquery.writeCapture-min.js",
            function () {
...
function GetQuoteComment(n) {
    ...
    $("#tbCommentBody").focus();
    $.getScript(location.protocol + "//common.cnblogs.com/script/encoder.js",
    function () {
...
function showImages(n) {
    ...
    function (n) {
        ...
        t.style.width = n.data.width + "px";
        t.style.height = n.data.height + "px";
        console.log(document.getElementById(n.data.iframeId).style.width)
    });
    $("#" + n + " img").each(function () {
        var n = this.src,
        u = /\.cnblogs\.com|\.cnitblog\.com|\.csdn\.net|\.codecogs\.com|\.360buyimg\.com/.test(n),
        f = n.indexOf("http") == 0,
        r;
        if (u || !f) {
            $(this).load(function () {
        ...
        var i = "iframe_" + Math.random(),
        n = n.indexOf("?") > 0 ? n + "&_=" + cb_entryId : n + "?_=" + cb_entryId,
        e = '<img id="img" src="' + n + '" style="border:none;max-width:' + t + "px\"><script>window.onload = function () {var img = document.getElementById('img'); window.parent.postMessage({iframeId:'" + i + "',width:img.width,height:img.height}, '" + location.origin + "');}<\/script>";
        iframeSrc = "data:text/html;charset=utf8," + encodeURI(e);
        r = $("<iframe />", {
            id: i,
            src: iframeSrc,
            style: "border:none",
...
function getBlogPostBody(n) {
    $.ajax({
        ...
        dataType: "text",
        success: function (t) {
            t ? ($("#postlist_postbody_" + n).html(t), cb_CodeHighlight(), cb_mathjax_render(), fixPostListBodyFormat(), n > 3861237 && $.getScript(location.protocol + "//common.cnblogs.com/highlight/9.1.0/highlight.min.js?id=20160127",
            function () {
...
function deliverAdT2() {
    try {
        $.ajax({
            url: location.protocol + "//ad-api.cnblogs.com/adunits/text/T2/creative",
            type: "get",
            dataType: "html",
            cache: !1,
            success: function (n) {
...
function deliverDirectAdC1() {
    $.ajax({
        url: location.protocol + "//ad-api.cnblogs.com/adunits/image/C1/creative",
        type: "get",
        dataType: "html",
        success: function (n) {
            $("#ad_c1").html(n)
...
function deliverAdC2() {
    $.ajax({
        url: location.protocol + "//ad-api.cnblogs.com/adunits/image/C2/creative",
        type: "get",
        dataType: "html",
        success: function (n) {
            $("#ad_c2").html(n)
...
function followByGroup(n, t) {
    loadScript(location.protocol + "//common.cnblogs.com/scripts/artDialog/dialog-plus-min.js",
    function () {
